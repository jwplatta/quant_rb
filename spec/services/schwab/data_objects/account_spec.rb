...
RSpec.describe DataObjects::Account do
  ...
  describe '.from_raw' do
    it 'creates an option chain object from raw data' do
      account = DataObjects::Account.build(raw_data)
      ...
      expect(account.positions.first).to be_an_instance_of DataObjects::Position
      expect(account.positions.first.instrument).to be_an_instance_of DataObjects::Instrument
      expect(account.initial_balances).to be_an_instance_of DataObjects::InitialBalances
      expect(account.current_balances).to be_an_instance_of DataObjects::CurrentBalances
      expect(account.projected_balances).to be_an_instance_of DataObjects::ProjectedBalances
    end
  end
end
