...
RSpec.describe Platypi::Schwab::DataObjects::Transaction do
  ...
  describe '.build' do
    it 'creates a transaction object from raw data' do
      transaction = Platypi::Schwab::DataObjects::Transaction.build(transaction_data)

      expect(transaction).to be_an_instance_of(Platypi::Schwab::DataObjects::Transaction)
      ...
      transaction.transfer_items.each do |item|
        expect(item).to be_an_instance_of(Platypi::Schwab::DataObjects::TransferItem)
        expect(item.instrument).to be_an_instance_of(Platypi::Schwab::DataObjects::Instrument)
      end
    end
  end
...
RSpec.describe Platypi::Schwab::DataObjects::TransferItem do
  ...
  describe '.build' do
    it 'creates a transfer item from raw data' do
      transfer_item = Platypi::Schwab::DataObjects::TransferItem.build(transfer_item_data)

      expect(transfer_item).to be_an_instance_of(Platypi::Schwab::DataObjects::TransferItem)
      ...
      expect(transfer_item.instrument).to be_an_instance_of(Platypi::Schwab::DataObjects::Instrument)
  ...
  describe '#fee?' do
    ...
    it 'returns true for fee types' do
      ...
      transfer_item = Platypi::Schwab::DataObjects::TransferItem.build(fee_data)
  ...
  describe '#commission?' do
    ...
    it 'returns true when fee_type is COMMISSION' do
      ...
      transfer_item = Platypi::Schwab::DataObjects::TransferItem.build(commission_data)
