...
RSpec.describe OptionsTrader::Schwab::DataObjects::OrderLeg do
  let(:raw_data) do
  ...
  describe '.build' do
    it 'creates an order leg object from raw data' do
      order_leg = OptionsTrader::Schwab::DataObjects::OrderLeg.build(raw_data)

      expect(order_leg).to be_an_instance_of(OptionsTrader::Schwab::DataObjects::OrderLeg)
      ...
      expect(order_leg.instrument).to be_an_instance_of(OptionsTrader::Schwab::DataObjects::Instrument)
    end
  end
