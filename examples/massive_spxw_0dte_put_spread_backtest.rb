...
class MassiveSpxwZeroDtePutSpreadExample < QuantRb::Strategy
  def initialize
  ...
  def open_position_if_needed(chains_by_expiry)
    ...
    info("Opened massive interpolated 0DTE SPXW put spread expiry=#{expiry} short=#{short_put.strike} long=#{long_put.strike} credit=#{limit_price}")
  end
...
QuantRb.logger.info("Running massive interpolated SPXW 0DTE put spread example")
...
result = QuantRb::BacktestEngine.run(MassiveSpxwZeroDtePutSpreadExample)
