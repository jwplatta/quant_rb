...
START_DATE = Date.iso8601(ENV.fetch("START_DATE", "2026-05-01"))
END_DATE = Date.iso8601(ENV.fetch("END_DATE", "2026-05-01"))
...
class SchwabSpxwZeroDtePutSpreadExample < QuantRb::Strategy
  def initialize
  ...
  def open_position_if_needed(chains_by_expiry)
    ...
    info("Opened schwab sampled 0DTE SPXW put spread expiry=#{expiry} short=#{short_put.strike} long=#{long_put.strike} credit=#{limit_price}")
  end
...
QuantRb.logger.info("Running schwab sampled SPXW 0DTE put spread example")
...
result = QuantRb::BacktestEngine.run(SchwabSpxwZeroDtePutSpreadExample)
