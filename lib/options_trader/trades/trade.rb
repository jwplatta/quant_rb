...
module OptionsTrader
  module Trades
    ...
    TRADE_STATES = {
      ...
      adjust_open_order_error: 'ADJUST_OPEN_ORDER_ERROR',
      adjust_exited: 'ADJUST_EXITED',
      adjust_close_order_sent: 'ADJUST_CLOSE_ORDER_SENT',
      adjust_close_order_error: 'ADJUST_CLOSE_ORDER_ERROR',
    ...
    class Trade
      class << self
        def load(trade_id)
          trade = TradeJournal.last_event(trade_id)
          trade
        rescue Errno::ENOENT
          nil
        end
        ...
        def create_strategy_from_h(strategy_data)
          ...
          when 'callspread'
            OptionsTrader::CallSpread.from_h(strategy_data)
          when 'putspread'
            OptionsTrader::PutSpread.from_h(strategy_data)
          when 'ironcondor'
            OptionsTrader::IronCondor.from_h(strategy_data)
          when 'nullstrategy'
            OptionsTrader::NullStrategy.new
          else
      ...
      def initialize(
        ...
        @journal = TradeJournal.new(trade_id: @trade_id)
      ...
      def next
        if current_state == TRADE_STATES[:no_trade_found]
          find_strategy
      ...
      def check_open_order
        ...
        if paper_trading
          if order_manager.paper_accepted?
          ...
          if order_manager.filled?
            @current_state = TRADE_STATES[:trade_entered]
          elsif strategy.market_change? && (order_manager.working? || order_manager.failed?)
      ...
      def check_adjust_open_order
        ...
        if paper_trading
          if order_manager.paper_accepted?
            ...
            @current_state = TRADE_STATES[:adjust_open_order_error]
          end
        else
          order_manager.check_order_status
          if strategy.market_change? && (order_manager.working? || order_manager.failed?)
            order_manager.stop_working_order
            send_adjust_open_order # NOTE: retry order
          elsif order_manager.filled?
            @current_state = TRADE_STATES[:adjust_entered]
          elsif order_manager.working?
      ...
      def check_adjust_close_order
        ...
        if paper_trading
          ...
          if strategy.market_change? && (order_manager.working? || order_manager.failed?)
            ...
            @current_state = TRADE_STATES[:adjust_close_order_error]
          end
        end
      end
      ...
      def check_trade_progress_and_risk
        ...
        if progress.exit?(strategy, journal.trade_history)
          send_close_order
      ...
      def strategy_finder_factory
        OptionsTrader::StrategyFinderFactory
      end
    end
  end
end
