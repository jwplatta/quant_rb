        ...
        class << self
          def build(data)
            ...
            data.fetch(:callExpDateMap).each do |exp_date, options|
              ...
              options.each_value do |opts|
                opts.each do |option_data|
                  call_opts << Platypi::Schwab::DataObjects::Option.build(underlying_symbol, option_data)
                end
              end
            end
            ...
            data.fetch(:putExpDateMap).each do |exp_date, options|
              ...
              options.each_value do |opts|
                opts.each do |option_data|
                  put_opts << Platypi::Schwab::DataObjects::Option.build(underlying_symbol, option_data)
                end
              end
            end
